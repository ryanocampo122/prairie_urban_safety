...
-------------------
- Uses a single primary station for Winnipeg
...
def clean_and_standardize(df, mapping, timeframe):
    ...
    Description
    -----------
    Harmonizes column naming, converts values to numeric types, and
    ensures a consistent datetime index for downstream analysis.
    ...
    mapping : dict
        Mapping of original column names to standardized variables.
    timeframe : int
    ...
    Notes
    -----
    - Handles inconsistent formatting across ECCC datasets
    - Removes invalid timestamps
    - Prepares data for aggregation and modeling pipeline
    """
...
def download_raw_data(station_id, year, month, timeframe):
    ...
    - Returns (None, 0) if data is unavailable or invalid
    """
...
def print_summary_table(df, name, agg='D'):
    ...
    Notes
    -----
    Outputs summary statistics to console for validation.
    """
    if df.empty:
...
def process_city():
    """
    Execute full climate data pipeline for Winnipeg.
    ...
    - Outputs are saved for integration with crime and sentiment datasets
    """
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        for month in range(1, 13):
            # 1️ Download HOURLY data for RH
            ...
            # 2️ Download DAILY summary for Temp & Precip
    ...
    # 3️ Process Hourly RH → Daily Mean
